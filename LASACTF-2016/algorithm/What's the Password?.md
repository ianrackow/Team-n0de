...
<b>Solution:</b><br>
If you observe the substrings, no character appears twice in the same line. Assuming that there are no repeats in the password, you can execute this code, where ```a``` is the substrings read from the text file. Avoid that shortest common superstring dp crap. Voila.<br><br>
```python
a = a.split()
